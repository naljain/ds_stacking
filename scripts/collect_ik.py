...
def _grasp_xy(block_xy, arm, cfg):
...
def main():
    ...
    parser.add_argument("--seed",     type=int, default=42,
                        help="Random seed for block placement and collection jitter")
    ...
    from src.primitives import (primitive_target, PRIMITIVE_ORDER,
                                grasp_quat_from_block, DEFAULT_DOWN_QUAT)
    try:
    ...
    default_q = np.asarray(cfg["arms"][f"default_joints_{args.arm}"], dtype=float)
    ...
    print(f"[INFO] Lula q_goal seed policy={args.ik_seed_policy}")
    ...
    while len(all_demos) < args.n_demos and attempts < max_attempts:
        ...
        for block_idx, block_name in enumerate(block_names):
            ...
            pick_pos = noisy_pos.copy()
            pick_pos[:2] = _grasp_xy(pick_pos[:2], args.arm, cfg)
            ...
            for primitive in PRIMITIVE_ORDER:
                ...
                target_block_pos = (
                    pick_pos if primitive in ("reach", "grasp", "lift")
                    else noisy_pos
                )
                target_cart = primitive_target(
                    primitive=primitive,
                    block_pos=target_block_pos,
                    goal_xy=goal_xy,
                    goal_z=goal_z,
                    hover_h=hover_h,
                    lift_h=lift_h,
                ...
                def record(q_override=None, q_dot_override=None):
                    ...
                    prim_buf.append({
                        ...
                        "q_goal_seed_policy": args.ik_seed_policy,
                        "grasp_xy_offset": (
                            np.asarray(pick_pos[:2]) - np.asarray(noisy_pos[:2])
                        ),
                    })
                    prev_q = q
